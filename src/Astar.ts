    ...
    *findPath()
    {
        ...
        while(opens.count() > 0)
        {
            let n : Node = opens.pop(); //하나 빼오고
            //이게 현재 탐색노드가 된다.
            if(prevDom != null) {
                (prevDom as HTMLDivElement).classList.remove("now");
            }
            prevDom = MapManager.Instance.getDom(n.pos);
            (prevDom as HTMLDivElement).classList.add("now");
    ...
    findOpenList(currentNode:Node)
    {
        ...
        for(let i = -1; i <= 1; i++)
        {
            for(let j = -1; j <= 1; j++)
            {
                ...
                let nX = cX + j, nY = cY + i;
                let next:Position = new Position(nX, nY);

                let n = this.closeList.find(x => x.pos.equal(next));
                if(n !== undefined) continue; //이미 방문한 노드리스트에 있다면

                if(MapManager.Instance.canMove(next.x, next.y))
                {
                    //g를 새로 계산
                    let g:number = currentNode.G + Math.sqrt( Math.pow(cX - nX, 2) + Math.pow(cY - nY, 2));
    ...
    calcH(currentPos :Position): number
    {
        const end = this.end;
